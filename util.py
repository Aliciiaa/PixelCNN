...
def afficher_train_page_modele(device):
    ...
    if dataset == 'MNIST' : 
        in_channels = 1 
        out_channels = 256
        file_weight = '.\PixelCNN\PixelCNN_MNIST.pth'
    else : 
        in_channels = 3 
        out_channels = 3*256
        file_weight = '.\PixelCNN\PixelCNN_CIFAR-10.pth'
...
def completion(testset, pixel, model, device, p):
    ...
    if dataset=='MNIST':
        ...
        image_trunc_copy = image_trunc.squeeze()
        image_trunc_copy.cpu().detach().numpy()
        ...
        with st.empty():
            for i in np.arange(pixel,28):
                for j in range(28):
                    ...
                    # Créer une copie temporaire de image_trunc
                    
                    image_trunc[ :, i, j] = (pixel_value/255 - mean[0])/std[0] 
                    

                    image_trunc_plot[i, j] = (pixel_value/255 - mean[0])/std[0]
                    
                    
                    fig = plt.figure(figsize=(1, 1))
                    plt.imshow(image_trunc_plot, cmap = 'gray')
                    plt.title("prédiction de l'image")
                    plt.show()
                    plt.close()
                    st.pyplot(fig)
